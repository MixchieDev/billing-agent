...
const actionConfig: Record<string, { icon: any; color: string; bgColor: string; label: string }> = {
  ...
  INVOICE_VOIDED: { icon: Ban, color: 'text-foreground', bgColor: 'bg-muted', label: 'Invoice Voided' },
  ...
  INVOICE_PDF_GENERATED: { icon: FileText, color: 'text-foreground', bgColor: 'bg-muted', label: 'PDF Generated' },
  ...
  CONTRACT_SETTINGS_UPDATED: { icon: Settings, color: 'text-foreground', bgColor: 'bg-muted', label: 'Contract Settings Updated' },
...
const defaultActionConfig = { icon: Clock, color: 'text-foreground', bgColor: 'bg-muted', label: 'Activity' };
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-foreground font-mono text-sm">{log.entityId}</span>
            </div>
          </div>
          ...
          {log.ipAddress && (
            <div>
              ...
              <p className="text-foreground font-mono text-sm">{log.ipAddress}</p>
            </div>
          )}
          ...
          {log.details && Object.keys(log.details).length > 0 && (
            <div>
              ...
              <pre className="bg-muted border rounded-md p-3 text-sm text-foreground overflow-x-auto whitespace-pre-wrap">
